...
import { CATS, TODAY, type Defect, type DefectStatus, type Priority } from '@/data/domain'
...
function QaPage() {
  ...
  useEffect(() => {
    if (!search.id) return
    const hit = all.find((d) => d.id === search.id)
            ...
            {virtualizer.getVirtualItems().map((vi) => {
              ...
              return (
                <button
                  ...
                  className="absolute left-0 flex w-full cursor-pointer items-center gap-3 rounded-[11px] border border-line bg-card px-3.75 text-left transition-all hover:-translate-y-px hover:border-[#FF9A6B] hover:shadow-[0_4px_14px_rgba(20,24,28,0.06)]"
                >
                  <span
