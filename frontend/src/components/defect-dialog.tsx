...
import { PRI_LABEL, RECO, type Defect, type DefectStatus } from '@/data/domain'
import { cn } from '@/lib/utils'

const FLOW: DefectStatus[] = ['ღია', 'მიმდინარე', 'შემოწმებაზე', 'დახურული']
...
export function DefectDialog({ defect, onClose }: { defect: Defect | null; onClose: () => void }) {
  ...
  const st = status ?? defect.st
  const doneIdx = FLOW.indexOf(st)
  const next = FLOW[Math.min(doneIdx + 1, FLOW.length - 1)]!
  const canReopen = st === 'შემოწმებაზე' || st === 'დახურული'

  const advance = () => {
    if (st === 'დახურული') return
    setStatus(next)
    toast({
      kind: 'ok',
      ...
      <DialogContent className="max-w-225">
        ...
        <DialogFooter className="flex-wrap justify-start gap-2.5">
          <Button
            ...
            onClick={advance}
          >
            {st === 'დახურული' ? '✓ დახურულია' : `სტატუსი → ${next}`}
          </Button>
          {canReopen && (
            <Button
              variant="outline"
